$wins = 0;
$average = 0
Start-Process javac -ArgumentList "-cp game_engine.jar SamplaPlayer.java"
...
for($j = 1; $j -le 10; $j++) {
    ...
    for ($i = 1; $i -le 10; $i++) {
        if($i % 2 -eq 1){
            $seed = Get-Random;
            $consoleOutput = java -jar game_engine.jar 0 game.oth.OthelloGame $seed 10 3 2000 game.oth.players.GreedyPlayer SamplePlayer
            ...
            $consoleOutput = java -jar game_engine.jar 0 game.oth.OthelloGame $seed 10 3 2000 SamplePlayer game.oth.players.GreedyPlayer
